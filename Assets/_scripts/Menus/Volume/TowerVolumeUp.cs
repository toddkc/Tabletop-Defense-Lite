...
public class TowerVolumeUp : MonoBehaviour {
	...
	void Start(){
		if(PlayerPrefs.HasKey("feedback")==false){
			PlayerPrefs.SetInt ("feedback", 8);
		}
		currentVolume = PlayerPrefs.GetInt ("feedback", 8);
	...
	void OnTriggerStay(Collider coll)
	{
		if (coll.GetComponentInParent<VRTK_ControllerEvents> ().triggerPressed == true && clickable == true) 
		{
			clickable = false;
			timeStamp = Time.time;
			currentVolume = PlayerPrefs.GetInt ("feedback");
			if (currentVolume < 10) {
				currentVolume += 1;
				PlayerPrefs.SetInt ("feedback", currentVolume);
				Display (currentVolume);
			}
		}
	}
